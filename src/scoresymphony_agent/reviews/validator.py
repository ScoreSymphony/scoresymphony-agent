...
def validate_review(review: ReviewResult) -> None:
    ...
    if not review.task_id or not review.run_id or not review.attempt_id or not review.reviewer_id:
        raise ValueError("Review identity references are required")
    ...
    for finding in review.findings:
        ...
        if finding.review_round < 1:
            raise ValueError("finding review_round must be >= 1")
